	...
	class WhitelabelOptions{
		...
		function save_options() {
			...
			if (!isset($_POST['theme_options_nonce']) ||
				!wp_verify_nonce($_POST['theme_options_nonce'], 'theme_options_nonce_field')) {
		...
		function import_options() {
		    ...
		    try {
		        ...
		        wp_die('Error processing data');
		    }

		    // Validate option keys and values before importing
		    foreach ($decoded_data as $key => $value) {
		        // Verify option key prefix for scope limitation
		        if (strpos($key, $this->_prefix) !== 0) {
		            continue; // Skip options not belonging to our prefix
		        }
		        ...
		        update_option($sanitized_key, wp_unslash($sanitized_value));
		    }

		    wp_die('Import completed successfully');
		}
		...
		function restore_options() {
		    ...
		    // Validate input data
		    if (!isset($_POST['data']) || !is_array($_POST['data']) || empty($_POST['data'][0][0])) {
		        wp_die('Invalid data format');
		    }

		    // Get backup data with sanitized option name
		    $backup_option_name = sanitize_key($_POST['data'][0][0]);
		    $backup_data = get_option($backup_option_name);

		    if (empty($backup_data)) {
		        wp_die('Backup data not found');
		    }

		    try {
		        $decoded_data = json_decode($backup_data, true);
		        if (json_last_error() !== JSON_ERROR_NONE) {
		            wp_die('Invalid backup data format');
		        }
		    } catch (Exception $e) {
		        wp_die('Error processing backup data');
		    }

		    // Validate and restore each option
		    foreach ($decoded_data as $option) {
		        // Verify required fields exist
		        if (!isset($option['option']) || !isset($option['value'])) {
		            continue;
		        }

		        // Verify option belongs to our scope
		        if (strpos($option['option'], $this->_prefix) !== 0) {
		            continue;
		        }

		        // Sanitize option key and value
		        $sanitized_key = sanitize_key($option['option']);
		        $sanitized_value = wp_kses_post($option['value']);

		        // Update option with sanitized data
		        update_option($sanitized_key, wp_unslash($sanitized_value));
		    }

		    wp_die('Restore completed successfully');
		}

		/**
		 * Validates option key format and scope
		 * @param string $key Option key to validate
		 * @return boolean Validation result
 		*/
		private function validate_option_key($key) {
		    // Check if key is string and has valid prefix
		    if (!is_string($key) || strpos($key, $this->_prefix) !== 0) {
		        return false;
		    }

		    // Check for valid characters in key
		    if (!preg_match('/^[a-zA-Z0-9_-]+$/', str_replace($this->_prefix, '', $key))) {
		        return false;
		    }
		...
		private function validate_option_value($value) {
		    // Check for basic data types only
		    if (!is_scalar($value) && !is_array($value)) {
		        return false;
		    }

		    // For arrays, recursively validate all elements
		    if (is_array($value)) {
...
if ( ! class_exists( 'WhitelabelSidebars' ) ) {
	...
	function load_scripts() {
		...
		if ( get_current_screen()->id == 'appearance_page_sidebars' ) {
			...
			// Localized strings and security nonce for AJAX.
			wp_localize_script(
				'curly-sidebars',
				'js_data',
				array_merge(
					$js_data,
					array(
						'nonce' => wp_create_nonce( 'update_sidebars_nonce' ),
					)
				)
			);
	...
	function update_sidebars() {
		...
		// CSRF and capability checks for sidebar management.
		check_ajax_referer( 'update_sidebars_nonce', 'security' );
		...
		if ( $method == 'update' ) {
			...
			if ( !empty($name) ) {
				...
				if ( !$sidebars ) {
					...
					wp_send_json( array( $count, $name ) );

				} else {

					if ( !in_array( $name , $sidebars, true ) ) {
